function [measPdfNew,gridDimNew, gridStepNew, measGridNew, eigVectNew,xkf,pkf] = measPdfInterpRBGBF(measPdf,gridDim,center,eigVect,Q,sFactor,nx,Npa,measMean,measVar,F,xkf,pkf)
...
Fintx = griddedInterpolant(gridDim,reshape(xkf,Npa),"linear","none"); % interpolant
Fintp = griddedInterpolant(gridDim,reshape(pkf,Npa),"linear","none"); % interpolant
...
xkf = Fintx(inerpOn')'; % Interpolation
pkf = Fintp(inerpOn')'; % Interpolation

measPdfNew(isnan(measPdfNew)) = 0; % Zeros for extrapolation, otherwise artifacts would appear
xkf(isnan(xkf)) = 0; % Zeros for extrapolation, otherwise artifacts would appear
pkf(isnan(pkf)) = 0; % Zeros for extrapolation, otherwise artifacts would appear
pkf = reshape(pkf,1,1,[]);
