...
% TENSOR TOOLBOX FOR MATLAB NEEDED https://www.tensortoolbox.org
...
runFlags.LGbF_Standard = false;     % classic LGbF
runFlags.LGbF_Spectral = false;     % spectral LGbF
runFlags.RBPF = false;           % Rao-Blackewellized Particle Filter - failing for this Q
runFlags.PF = true;           % Bootstrap Particle Filter
runFlags.UKF = false;           % Unscented Kalman Filter
runFlags.LGbF_CPD = true;     % classic LGbF
...
while mc <= MC
    % disp(['Monte Carlo: ', num2str(mc), '/', num2str(MC)])
    try
        ...
        for k = 1:endTime-1
            ...
            if runFlags.LGbF_CPD
                ...
                [measMeanCPD(:,k), measVarCPDpom] = ...
                    momentsCPD(measPdfCPD, gridCPD, predGridDeltaCPD(:,k));

                measVarCPD(:,:,k) = diag(measVarCPDpom);

                % Advection
                [predDensityCPD, gridCPD, predGridDeltaCPD] = gbfAdvectionCPD(measPdfCPD, measMeanCPD(:,k), measVarCPDpom, ...
                    gridCPD, F, Q, NpaCPD, sFactorCPD, predGridDeltaCPD, Rdef, k);

                % Diffusion
                [predDensityProbCPD] = gbfDiffusionCPD(predDensityCPD, gridCPD, Q, predGridDeltaCPD, nx, k);
        ...
        if runFlags.LGbF_CPD
            [rmsePMFCPD(:,mc), astdPMFCPD(:,mc), annesPMFCPD(mc)] = calcRes(x, measMeanCPD, measVarCPD, k); %#ok<*SAGROW>
        ...
        disp(ME.message);  % Print error message
        % Optional: display full stack
        disp('Error stack:');
        disp(ME.stack);
    end
...
if runFlags.LGbF_CPD
    annes_PMFoutPomCPD  = sum(annesPMFCPD)  / (nx * mc * (k + 1));
    rmsePMFoutCPD      = mean(rmsePMFCPD , 2);
    astdPMFoutCPD       = mean(astdPMFCPD , 2);
    tocPMFavgOutCPD     = mean(tocPMFavgCPD);
end
...
if runFlags.LGbF_CPD
    rowNames{end+1} = 'LGbF CPD';
    vals{end+1} = [rmsePMFoutCPD(1), rmsePMFoutCPD(2), rmsePMFoutCPD(3), ...
        astdPMFoutCPD(1), astdPMFoutCPD(2), astdPMFoutCPD(3), ...
        annes_PMFoutPomCPD, tocPMFavgOutCPD];
end
...
if runFlags.LGbF_CPD, plot(measMeanCPD(1,:),measMeanCPD(2,:)); end
